...
<template>
    <div class="relative w-full max-w-2xl mx-auto" ref="searchContainer">
        <form @submit.prevent="submitSearch" class="flex items-center border-2 border-indigo-500 rounded-xl overflow-hidden bg-white shadow-sm relative z-20">
            <input v-model="searchQuery" @focus="searchQuery.length >= 2 ? isOpen = true : null" type="text" placeholder="Tìm kiếm sản phẩm, thương hiệu..."
                class="flex-1 pl-4 pr-12 py-2.5 text-sm focus:outline-none text-gray-700 placeholder-gray-400 border-none ring-0" />
        ...
        <Transition enter-active-class="transition duration-200 ease-out" enter-from-class="opacity-0 -translate-y-2" leave-active-class="transition duration-150 ease-in" leave-to-class="opacity-0 -translate-y-2">
            <div v-if="isOpen && searchQuery.length >= 2" 
                ...
                <div v-if="results.length > 0">
                    ...
                    <ul class="max-h-[350px] overflow-y-auto scrollbar-thin scrollbar-thumb-gray-200">
                        <li v-for="prod in results" :key="prod.id">
                            <Link :href="route('client.product.detail', prod.slug)" @click="isOpen = false"
                                class="flex items-center gap-3 p-3 hover:bg-indigo-50 transition border-b border-gray-50 last:border-0 group">
                                <img :src="prod.thumbnail || 'https://placehold.co/80x80/f8fafc/94a3b8?text=IMG'" 
                                     class="w-12 h-12 rounded object-cover border border-gray-100 bg-white group-hover:border-indigo-200 transition">
                                <div class="flex-1 min-w-0">
                                    <h4 class="text-sm font-bold text-gray-800 truncate group-hover:text-indigo-600 transition">{{ prod.name }}</h4>
                                    <div class="flex items-center gap-2 mt-0.5">
                                        <span class="text-red-600 font-black text-sm">{{ vnd(prod.sale_price || prod.base_price) }}</span>
                                        <del v-if="prod.sale_price" class="text-xs text-gray-400">{{ vnd(prod.base_price) }}</del>
                                    </div>
                                </div>
                            </Link>
                        </li>
                    </ul>
                    <button @click="submitSearch" class="w-full py-2.5 bg-gray-50 hover:bg-gray-100 text-indigo-600 text-sm font-bold border-t border-gray-100 transition text-center">
                ...
                <div v-else-if="!isLoading" class="p-6 text-center text-gray-500">
                    ...
                    <p class="text-sm">Không tìm thấy sản phẩm nào khớp với "<strong class="text-gray-800">{{ searchQuery }}</strong>"</p>
                </div>
            </div>
        </Transition>
    </div>
