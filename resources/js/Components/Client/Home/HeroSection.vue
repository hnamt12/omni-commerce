...
<template>
    <div class="flex flex-col gap-3">
        <div class="flex gap-3 h-[380px]">
            ...
            <div class="w-64 bg-white rounded-xl shadow-sm border border-gray-100 hidden lg:flex flex-col overflow-visible shrink-0 relative" @mouseleave="hoveredCategory = null">
                <div class="bg-gray-50 px-4 py-2.5 border-b border-gray-100 font-black text-sm text-gray-800 uppercase tracking-wider rounded-t-xl">Danh mục</div>
                ...
                <div v-if="hoveredCategory" 
                     class="absolute left-full top-0 w-[900px] h-[380px] bg-white rounded-r-2xl shadow-[30px_0_50px_-15px_rgba(0,0,0,0.15)] border border-gray-100 border-l-0 z-[100] p-6 flex gap-6">
                     
                    <div class="flex-1 grid grid-cols-2 gap-8">
                        <div>
                            <h4 class="text-xs font-black text-gray-900 mb-4 uppercase tracking-wider">Thương hiệu</h4>
                            <div class="flex flex-wrap gap-2 mb-8">
                                <Link v-for="brand in hoveredCategory.top_brands" :key="brand.id" :href="route('client.shop', { brand_id: brand.id, category_id: hoveredCategory.id })" 
                                      class="text-xs font-semibold text-gray-600 bg-gray-50 border border-gray-200 hover:border-red-400 hover:text-red-500 rounded-lg px-2.5 py-1.5 transition">
                                    {{ brand.name }}
                                </Link>
                                <span v-if="!hoveredCategory.top_brands?.length" class="text-xs text-gray-400 italic">Đang cập nhật...</span>
                            </div>

                            <h4 class="text-xs font-black text-gray-900 mb-4 uppercase tracking-wider">Mức giá</h4>
                        ...
                        <div>
                            <h4 class="text-xs font-black text-gray-900 mb-4 uppercase tracking-wider">Nhu cầu</h4>
                            <ul class="space-y-3 text-sm text-gray-600 font-medium">
                    ...
                    <div class="w-[300px] border-l border-gray-100 pl-6 flex flex-col">
                        ...
                        <div class="space-y-4 flex-1">
                            <Link v-for="prod in hoveredCategory.top_products" :key="prod.id" :href="route('client.product.detail', prod.slug)" 
                                  class="flex gap-3 group bg-white p-2 rounded-xl hover:bg-gray-50 transition border border-transparent hover:border-gray-200">
                                <img :src="prod.thumbnail" class="w-16 h-16 rounded-lg border border-gray-100 bg-white object-cover shrink-0">
                                <div class="flex-1 min-w-0">
                                    <p class="text-xs font-bold text-gray-800 group-hover:text-red-500 line-clamp-2 leading-snug">{{ prod.name }}</p>
                                    <p class="text-red-600 font-black text-sm mt-1.5">{{ vnd(prod.sale_price || prod.base_price) }}</p>
                                </div>
                            </Link>
