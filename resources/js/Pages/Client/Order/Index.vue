            ...
            <div class="max-w-[1200px] mx-auto px-4 flex flex-col md:flex-row gap-6">
                ...
                <div class="w-full md:w-64 shrink-0">
                    <div class="flex items-center gap-3 mb-6 bg-white p-4 rounded-2xl shadow-sm border border-gray-100">
                        ...
                        <div>
                            <p class="font-bold text-gray-900">{{ $page.props.auth.customer.name }}</p>
                    ...
                    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden py-2">
                        <nav class="space-y-1">
                            <Link :href="route('client.profile')"
                                class="block w-full text-left px-5 py-3 text-sm text-gray-700 hover:bg-gray-50 hover:text-indigo-600 transition">
                                👤 Thông tin cá nhân
                            </Link>
                            <Link :href="route('client.order.index')"
                                class="block w-full text-left px-5 py-3 text-sm font-bold text-indigo-600 bg-indigo-50 border-r-4 border-indigo-600 transition">
                                📦 Đơn hàng của tôi
                            </Link>
                            <Link :href="route('client.logout')" method="post" as="button"
                                class="block w-full text-left px-5 py-3 text-sm text-gray-700 hover:bg-gray-50 hover:text-red-600 transition">
                                🚪 Đăng xuất
                            </Link>
                        </nav>
                    </div>
                ...
                <div class="flex-1 space-y-4">
                    ...
                    <div v-if="orders.data.length === 0"
                        ...
                        <h3 class="text-lg font-bold text-gray-900 mb-2">Chưa có đơn hàng nào</h3>
                        <p class="text-gray-500 text-sm mb-6">Bạn chưa thực hiện giao dịch nào trên hệ thống.</p>
                    ...
                    <div v-for="order in orders.data" :key="order.id"
                        ...
                        <div class="px-5 py-3 border-b border-gray-100 flex justify-between items-center bg-gray-50/50">
                            <div class="flex items-center gap-2">
                                <span class="font-bold text-gray-800 flex items-center gap-2">🛒 OmniCommerce</span>
                        ...
                        <div class="px-5 py-4">
                            <div v-for="(item, index) in order.items" :key="item.id">
                                <div v-if="index === 0" class="flex gap-4">
                                    ...
                                    <div class="text-right">
                                        <p class="font-black text-gray-900">{{ vnd(item.price) }}</p>
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <!-- Card footer -->
                        <div class="px-5 py-4 border-t border-gray-100 bg-red-50/10 flex flex-col md:flex-row justify-between items-end md:items-center gap-4">
                            <div class="text-sm text-gray-500">Ngày đặt: {{ formatDate(order.created_at) }}</div>
                            <div class="flex items-center gap-4 flex-wrap justify-end">
                                <div class="text-sm">Thành tiền: <span class="text-xl font-black text-red-600">{{ vnd(order.grand_total) }}</span></div>
                    ...
                    <div v-if="orders.last_page > 1" class="flex flex-wrap gap-1 justify-center pt-2">
                        <template v-for="(link, i) in orders.links" :key="i">
                            ...
                            <span v-else v-html="link.label"
                                class="px-3 py-1.5 rounded-lg text-sm text-gray-400 border border-gray-100 bg-gray-50 cursor-not-allowed" />
                        </template>
                    </div>
