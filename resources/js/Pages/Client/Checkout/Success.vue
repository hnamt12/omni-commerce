...
<template>
    ...
    <ClientLayout>
        <div class="bg-[#f8f9fa] dark:bg-slate-950 min-h-[85vh] py-10 px-4 flex items-center justify-center transition-colors duration-300">

            <div class="max-w-5xl w-full mx-auto bg-white dark:bg-slate-900 rounded-3xl shadow-sm border border-gray-100 dark:border-slate-800 overflow-hidden flex flex-col lg:flex-row">
                ...
                <div class="lg:w-[60%] p-8 lg:p-12 flex flex-col justify-center">
                    ...
                    <div class="flex items-center gap-4 mb-8">
                        ...
                        <div>
                            <h1 class="text-3xl font-black text-gray-900 dark:text-gray-100 tracking-tight">Đặt hàng thành công!</h1>
                            <p class="text-sm text-gray-500 mt-1">Mã đơn hàng: <span class="font-bold text-indigo-600">#{{ order.order_code }}</span></p>
                        </div>
                    </div>

                    <p class="text-gray-600 dark:text-gray-400 mb-8 leading-relaxed">
                        Cảm ơn bạn đã mua sắm tại <span class="font-bold text-gray-900 dark:text-gray-100">OmniCommerce</span>.
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-10">
                        ...
                        <div class="bg-white dark:bg-slate-800 p-5 rounded-2xl border border-gray-200 dark:border-slate-700 shadow-sm">
                            ...
                            <p class="text-sm text-gray-600 dark:text-gray-400 mt-1 leading-relaxed">{{ order.address }}</p>
                        </div>
                ...
                <!-- ── RIGHT: Order Summary ─────────────────────────────── -->
                <div class="lg:w-[40%] bg-gray-50/50 dark:bg-slate-800/50 border-t lg:border-t-0 lg:border-l border-gray-100 dark:border-slate-800 p-8 lg:p-12 flex flex-col">
                    <h3 class="text-lg font-black text-gray-900 dark:text-gray-100 mb-6">Tóm tắt đơn hàng</h3>
                    ...
                    <div class="flex-1 overflow-y-auto max-h-[280px] pr-2 space-y-4 mb-6 scrollbar-thin scrollbar-thumb-gray-200">
                        <div v-for="item in order.items" :key="item.id" class="flex gap-3">
                            <div class="w-16 h-16 bg-white dark:bg-slate-900 border border-gray-200 dark:border-slate-700 rounded-xl overflow-hidden shrink-0 relative">
                            ...
                            <div class="text-right shrink-0">
                                <p class="text-sm font-black text-gray-900 dark:text-gray-100">{{ vnd(item.price) }}</p>
                            </div>
                        </div>
                    </div>

                    <!-- Price summary -->
                    <div class="border-t border-gray-200 dark:border-slate-700 pt-5 space-y-3">
                        ...
                        <div class="flex justify-between items-center pt-4 border-t border-gray-200 dark:border-slate-700">
                            ...
                            <span class="text-3xl font-black text-red-600 dark:text-red-400">{{ vnd(order.grand_total) }}</span>
                        </div>
                    </div>
                </div>
