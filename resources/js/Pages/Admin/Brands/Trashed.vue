...
const handleSearch = () => {
    router.get(route('admin.brands.trashed'), { search: search.value }, { preserveState: true, replace: true });
};
...
const forceDeleteBrand = () => {
    if (itemToDelete.value) {
        router.delete(route('admin.brands.force-delete', itemToDelete.value), {
            preserveScroll: true,
...
<template>
    <Head title="Thương hiệu đã xóa" />
            ...
            <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
                <div>
                    <h1 class="text-2xl font-bold text-gray-900 dark:text-white">Thùng rác: Thương hiệu</h1>
                    <p class="text-gray-500 dark:text-gray-400 mt-1 text-sm">Quản lý các thương hiệu đã xóa. Bạn có thể khôi phục hoặc xóa vĩnh viễn.</p>
                </div>
                <div class="flex items-center gap-3">
                    ...
                    <Link :href="route('admin.brands.index')" class="px-4 py-2 bg-white dark:bg-slate-800 border border-gray-300 dark:border-slate-600 rounded-md text-sm font-medium text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-slate-700 transition-colors shadow-sm flex items-center">
                    ...
                    <table class="w-full text-left border-collapse">
                        ...
                        <tbody class="divide-y divide-gray-100 dark:divide-slate-700 text-sm text-gray-700 dark:text-gray-300">
                            <tr v-for="brand in brands.data" :key="brand.id" class="hover:bg-gray-50 dark:hover:bg-slate-800/50 transition-colors">
                                ...
                                <td class="p-4 font-medium text-gray-900 dark:text-white flex items-center gap-3 opacity-60">
                                ...
                                <td class="p-4 text-gray-500 dark:text-gray-400 opacity-60">{{ brand.slug }}</td>
                                <td class="p-4 text-center">
                                    <span class="px-2.5 py-1 bg-red-100 dark:bg-red-900/30 text-red-700 dark:text-red-400 rounded-full text-xs font-medium">Đã xóa</span>
                                </td>
                                <td class="p-4 text-right">
                                    <button @click="restoreBrand(brand.id)" class="text-green-600 dark:text-green-400 hover:text-green-900 dark:hover:text-green-300 font-medium mr-3 transition-colors">Khôi phục</button>
                                    <button @click="confirmDelete(brand.id)" class="text-red-600 dark:text-red-400 hover:text-red-900 dark:hover:text-red-300 font-medium transition-colors border border-red-200 dark:border-red-900 px-2 py-1 rounded">Xóa vĩnh viễn</button>
                                </td>
                            </tr>
            ...
            <div class="p-5">
                <div class="flex items-start gap-4">
                    ...
                    <div>
                        ...
                        <p class="mt-2 text-sm text-gray-500 dark:text-gray-400">Hành động này <strong class="text-red-500">KHÔNG THỂ MANG LẠI DỮ LIỆU</strong>. File logo trên máy chủ cũng sẽ bị xóa. Bạn có chắc chắn?</p>
                    </div>
                </div>
                <div class="mt-6 flex justify-end gap-3">
                    <button @click="closeDeleteModal" class="px-4 py-2 bg-white dark:bg-slate-800 border border-gray-300 dark:border-slate-600 rounded-md text-sm font-medium text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-slate-700 transition-colors">Hủy bỏ</button>
                    <button @click="forceDeleteBrand" class="px-4 py-2 bg-red-600 hover:bg-red-700 text-white rounded-md text-sm font-medium transition-colors">Xóa Vĩnh Viễn</button>
                </div>
            </div>
        </Modal>
    </template>
