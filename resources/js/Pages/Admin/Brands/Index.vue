            ...
            <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
                ...
                <div class="flex items-center gap-3">
                    <div class="relative">
                        ...
                        <input v-model="search" type="text" placeholder="Tìm kiếm thương hiệu..." class="pl-9 pr-4 py-2 border border-gray-200 dark:border-slate-700 bg-white dark:bg-slate-800 rounded-md focus:outline-none focus:ring-2 focus:ring-indigo-500 dark:focus:ring-indigo-400 text-sm text-gray-900 dark:text-gray-100 placeholder-gray-400 dark:placeholder-gray-500 w-full md:w-64 transition-colors">
                    </div>
                ...
                <div class="overflow-x-auto">
                    <table class="w-full text-left border-collapse">
                        <thead>
                            <tr class="bg-gray-50 dark:bg-slate-700/50 text-gray-500 dark:text-gray-400 text-xs uppercase tracking-wider border-b border-gray-200 dark:border-slate-700">
                                ...
                                <th class="p-4 font-medium cursor-pointer hover:bg-gray-100 dark:hover:bg-slate-700" @click="handleSort('name')">
                                    Thương hiệu <span v-if="filters?.sort === 'name'">{{ filters?.direction === 'asc' ? '↑' : '↓' }}</span>
                                </th>
                        ...
                        <tbody class="divide-y divide-gray-100 dark:divide-slate-700 text-sm text-gray-700 dark:text-gray-300">
                            <tr v-for="brand in brands.data" :key="brand.id" @click="router.get(route('admin.brands.show', brand.id))" class="hover:bg-gray-50 dark:hover:bg-slate-800/50 transition-colors cursor-pointer hover:border-indigo-400">
                                ...
                                <td class="p-4 text-right">
                                    <div class="flex items-center justify-end gap-1.5" @click.stop>
                                        <button @click.stop="router.get(route('admin.brands.show', brand.id))" class="p-2 text-blue-500 hover:bg-blue-50 rounded-lg transition-colors" title="Xem chi tiết">
                                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" />
                                            </svg>
                                        </button>
                                        <button @click.stop="router.get(route('admin.brands.edit', brand.id))" class="p-2 text-amber-500 hover:bg-amber-50 rounded-lg transition-colors" title="Sửa">
                                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" /></svg>
                                        </button>
                                        <button @click.stop="deleteBrand(brand.id)" class="p-2 text-red-500 hover:bg-red-50 rounded-lg transition-colors" title="Xóa">
