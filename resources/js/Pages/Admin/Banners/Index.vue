...
<template>
    ...
    <div class="max-w-7xl mx-auto pb-10">
        ...
        <div class="mb-6 flex items-center justify-between">
            <div>
                <h1 class="text-2xl font-bold text-gray-900">🖼️ Quản lý Banner</h1>
                <p class="text-gray-500 mt-1 text-sm">Cấu hình layout và vị trí hiển thị Banner trên trang khách.</p>
            </div>
        ...
        <div class="mt-2">
            ...
            <!-- Info notice -->
            <div class="bg-blue-50 border border-blue-100 p-4 rounded-xl mb-5 flex gap-3">
                ...
                <div>
                    <p class="font-bold text-blue-800 text-sm">Lưu ý khu vực {{ currentTabObj.name }}</p>
                    <p class="text-sm text-blue-600 mt-0.5">{{ currentTabObj.description }}</p>
                </div>
            </div>

            <!-- Grid Cards -->
            <div v-if="currentBanners.length" class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-4">
                <div v-for="(b, index) in currentBanners" :key="b.id"
                    ...
                    class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden flex flex-col transition hover:shadow-md cursor-move group/card">

                    <!-- Image Preview -->
                    <div class="h-32 bg-gray-50 border-b border-gray-100 relative group pointer-events-none overflow-hidden">
                        ...
                        <div class="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition flex items-center justify-center gap-3 pointer-events-auto">
                            <Link :href="route('admin.banners.edit', b.id)"
                                class="p-1.5 bg-white rounded-lg text-indigo-600 hover:bg-indigo-50 shadow">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/></svg>
                            </Link>
                            <button @click="deleteBanner(b.id)"
                                class="p-1.5 bg-white rounded-lg text-red-600 hover:bg-red-50 shadow">
                    ...
                    <div class="p-3 flex-1 flex flex-col justify-between">
                        <div>
                            <div class="flex justify-between items-start mb-2">
                                <h3 class="font-bold text-gray-900 line-clamp-1">{{ b.title }}</h3>
                                <span class="text-[10px] font-bold text-gray-500 bg-gray-100 border border-gray-200 px-2 py-0.5 rounded ml-2 shrink-0 flex items-center gap-1">
                                    <svg class="w-3 h-3 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 9l4-4 4 4m0 6l-4 4-4-4"/></svg>
                                    Kéo thả
                                </span>
                            </div>
                        </div>
                        ...
                        <div class="flex items-center justify-between pt-3 border-t border-gray-100">
                            <span class="text-xs font-bold" :class="b.is_active ? 'text-green-600' : 'text-gray-400'">
                                {{ b.is_active ? 'Đang hiển thị' : 'Đã ẩn' }}
                            </span>
                            <button @click="toggleActive(b)"
                                class="relative inline-flex h-5 w-9 items-center rounded-full transition-colors"
                                :class="b.is_active ? 'bg-green-500' : 'bg-gray-200'">
            ...
            <!-- Empty state -->
            <div v-else class="bg-white rounded-xl border border-gray-100 border-dashed p-12 text-center">
                <p class="text-4xl mb-3">📬</p>
                <h3 class="text-lg font-bold text-gray-900">Không có banner nào ở khu vực này</h3>
                <p class="text-gray-500 text-sm mt-1 mb-5">Vui lòng thêm banner mới và chọn vị trí "{{ currentTabObj.name }}" để hiển thị.</p>
                <Link :href="route('admin.banners.create')" class="text-indigo-600 bg-indigo-50 px-5 py-2.5 rounded-lg text-sm font-bold hover:bg-indigo-100 transition">
                    + Thêm Banner Mới
                </Link>
            </div>
        </div>
