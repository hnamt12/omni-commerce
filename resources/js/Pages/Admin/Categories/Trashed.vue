...
const handleSearch = () => {
    router.get(route('admin.categories.trashed'), { search: search.value }, { preserveState: true, replace: true });
};
...
const forceDeleteCategory = () => {
    if (itemToDelete.value) {
        router.delete(route('admin.categories.force-delete', itemToDelete.value), {
            preserveScroll: true,
...
<template>
    <Head title="Danh mục đã xóa" />

            <div class="w-full">
            <!-- Header -->
            <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
                <div>
                    <h1 class="text-2xl font-bold text-gray-900 dark:text-white">Thùng rác: Danh mục</h1>
                    <p class="text-gray-500 dark:text-gray-400 mt-1 text-sm">Quản lý các danh mục đã xóa. Bạn có thể khôi phục nó.</p>
                </div>
                <div class="flex items-center gap-3">
                    ...
                    <Link :href="route('admin.categories.index')" class="px-4 py-2 bg-white dark:bg-slate-800 border border-gray-300 dark:border-slate-600 rounded-md text-sm font-medium text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-slate-700 transition-colors shadow-sm flex items-center">
                    ...
                    <table class="w-full text-left border-collapse">
                        ...
                        <tbody class="divide-y divide-gray-100 dark:divide-slate-700 text-sm text-gray-700 dark:text-gray-300">
                            <tr v-for="category in categories.data" :key="category.id" class="hover:bg-gray-50 dark:hover:bg-slate-800/50 transition-colors">
                                ...
                                <td class="p-4 font-medium text-gray-900 dark:text-white opacity-60"><del>{{ category.name }}</del></td>
                                ...
                                <td class="p-4 text-gray-500 dark:text-gray-400 opacity-60">{{ category.slug }}</td>
                                ...
                                <td class="p-4 text-right">
                                    <button @click="restoreCategory(category.id)" class="text-green-600 dark:text-green-400 hover:text-green-900 dark:hover:text-green-300 font-medium mr-3 transition-colors">Khôi phục</button>
                                    <button @click="confirmDelete(category.id)" class="text-red-600 dark:text-red-400 hover:text-red-900 dark:hover:text-red-300 font-medium transition-colors border border-red-200 dark:border-red-900 px-2 py-1 rounded">Xóa vĩnh viễn</button>
                                </td>
                            </tr>
            ...
            <div class="p-5">
                <div class="flex items-start gap-4">
                    ...
                    <div>
                        ...
                        <p class="mt-2 text-sm text-gray-500 dark:text-gray-400">Bạn có chắc chắn muốn xóa bản ghi này? Toàn bộ dữ liệu liên quan sẽ biến mất VĨNH VIỄN.</p>
                    </div>
                </div>
                <div class="mt-6 flex justify-end gap-3">
                    ...
                    <button @click="forceDeleteCategory" class="px-4 py-2 bg-red-600 hover:bg-red-700 text-white rounded-md text-sm font-medium transition-colors">Xác nhận Xóa</button>
                </div>
            </div>
        </Modal>
    </template>
