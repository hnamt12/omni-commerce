        ...
        try {
            DB::transaction(function () use ($request) {
                foreach ($request->items as $item) {
                    // Skip no-op entries
                    if (in_array($item['type'], ['add', 'sub']) && $item['quantity'] == 0) continue;
                    ...
                    if ($stockBefore === $stockAfter) continue; // No change – skip log
    ...
    public function updateStock(Request $request, $variantId)
    {
        ...
        try {
            DB::transaction(function () use ($request, $variantId) {
                ...
                if ($type === 'add')       { $stockAfter = $stockBefore + $quantity; $quantityChange = $quantity;           $actionType = 'import'; }
                elseif ($type === 'sub')   { $stockAfter = $stockBefore - $quantity; $quantityChange = -$quantity;          $actionType = 'export'; }
                elseif ($type === 'set')   { $stockAfter = $quantity;                $quantityChange = $stockAfter - $stockBefore; $actionType = 'adjustment'; }

                if ($stockAfter < 0) throw new \Exception('Số lượng tồn kho không thể âm.');
